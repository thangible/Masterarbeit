...
% To maintain the foundational knowledge of the pre-trained SAM while specializing it for meteorological features, we use Low-Rank Adaptation (LoRA). 
...
% By employing LoRA, we isolate the adaptation process of the encoder. This methodology ensures that the model focuses exclusively on learning to extract high-quality, diagnostic features from the 16 multi-parameter atmospheric grids. This is vital for recognizing the "ribbon-like" corridors of ARs and the "circular" structures of TCs, which differ significantly from the high-frequency edges found in traditional RGB images.
